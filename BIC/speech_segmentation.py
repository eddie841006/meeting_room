...
def multi_segmentation(y, sr, frame_size, frame_shift,save_file, plot_seg=False, save_seg=False, cluster_method=None):
    # y, sr = librosa.load(file, sr=sr)

    mfccs = librosa.feature.mfcc(y, sr, n_mfcc=12, hop_length=frame_shift, n_fft=frame_size)
    seg_point = speech_segmentation(mfccs / mfccs.max())# 1081202
...
#    rangeLoop = range(len(seg_point) - 1)
    rangeLoop = range(0,len(seg_point)-1)# 1081202
    output_segpoint = []
    for i in rangeLoop:
...
#            librosa.output.write_wav("save_audio/%s.wav" % i, tempAudio, 16000)
 
#            librosa.output.write_wav("save_audio/%s.wav" % i, (tempAudio* maxv).astype(np.int16), sr)
            sf.write(save_file+"/%s.wav" % i, tempAudio, 16000, subtype='PCM_16')
